...
static size_t print_tree(size_t offset, size_t window, const char *shift, size_t level, size_t base_index, size_t selected_entry,
    ...
    struct menu_entry *entry = current_entry;
    while (entry != NULL) {
        cur_len = 0;
        ...
        if (!no_print && base_index + max_entries >= offset && base_index + max_entries < offset + window) {
            if (!no_print) print("%s", shift);
            if (level) {
                for (size_t i = level - 1; i > 0; i--) {
                    struct menu_entry *actual_parent = entry;
            ...
            if (entry->sub) {
                if (!no_print) print(entry->expanded ? "[-]" : "[+]");
            } else if (level) {
                if (!no_print) print(serial ? "-->" : "──►");
            } else {
                if (!no_print) print("   ");
...
noreturn void _menu(bool first_run) {
    ...
    size_t active_memmap_entries = 0;
    struct memmap_entry *active_memmap = get_raw_memmap(&active_memmap_entries);
    ...
    if (rewound_memmap != NULL) {
...
#endif
        memcpy(active_memmap, rewound_memmap, rewound_memmap_entries * sizeof(struct memmap_entry));
        active_memmap_entries = rewound_memmap_entries;
    } else {
        ...
        rewound_memmap = ext_mem_alloc((active_memmap_entries + 16) * sizeof(struct memmap_entry));
        memcpy(rewound_memmap, active_memmap, active_memmap_entries * sizeof(struct memmap_entry));
        rewound_memmap_entries = active_memmap_entries;
