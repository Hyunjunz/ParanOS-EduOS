...
#ifndef LIMINE_VERSION
#define LIMINE_VERSION "unknown"
#endif
...
static uint64_t get_hhdm_span_top(int base_revision) {
    size_t memmap_entries_local = 0;
    struct memmap_entry *memmap_local = get_raw_memmap(&memmap_entries_local);

    uint64_t ret = base_revision >= 3 ? 0 : 0x100000000;
    for (size_t i = 0; i < memmap_entries_local; i++) {
        if (((base_revision >= 1 && base_revision < 3) || base_revision >= 4) && (
            memmap_local[i].type == MEMMAP_RESERVED
         || memmap_local[i].type == MEMMAP_BAD_MEMORY)) {
            continue;
        }

        if (base_revision == 3 && (
            memmap_local[i].type != MEMMAP_USABLE
         && memmap_local[i].type != MEMMAP_BOOTLOADER_RECLAIMABLE
         && memmap_local[i].type != MEMMAP_KERNEL_AND_MODULES
         && memmap_local[i].type != MEMMAP_FRAMEBUFFER
         && memmap_local[i].type != MEMMAP_EFI_RECLAIMABLE)) {
            continue;
        }

        uint64_t base = memmap_local[i].base;
        uint64_t length = memmap_local[i].length;
...
FEAT_START
    ...
    if (pm_request == NULL)
        break;
    if (pm_request->revision == 0
     && pm_request->mode == 0
     && pm_request->min_mode == 0
     && pm_request->max_mode == 0) {
        // 요청이 정의되지 않은 커널이면 무시하고 기본 모드로 진행
        break;
    }
    ...
    if (pm_request->revision >= 1) {
        uint64_t req_min = pm_request->min_mode;
        uint64_t req_max = pm_request->max_mode;

        // If the executable didn't fill these, fall back to the broadest range.
        if (req_min == 0) {
            req_min = PAGING_MODE_MIN;
        }
        if (req_max == 0 || req_max < req_min) {
            req_max = req_min;
        }

        kern_min_mode = (int)paging_mode_limine_to_vmm(req_min);
        kern_max_mode = (int)paging_mode_limine_to_vmm(req_max);
    }

    // Ensure loader minimum does not exceed what the executable can handle.
    if (min_supported_paging_mode > kern_max_mode) {
        min_supported_paging_mode = kern_max_mode;
    }
    if (max_supported_paging_mode < min_supported_paging_mode) {
        max_supported_paging_mode = min_supported_paging_mode;
    }
...
FEAT_END
    ...
    if (base_revision < 3) {
        ...
        pmm_sanitise_entries(memmap_local, &memmap_entries_local, true);
    }
