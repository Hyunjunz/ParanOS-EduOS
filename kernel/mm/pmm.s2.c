...
void init_memmap(void) {
    ...
    // Cache total pages for later runtime queries
    uint64_t total = 0;
    for (size_t i = 0; i < memmap_entries; i++) {
        if (memmap[i].type == MEMMAP_USABLE ||
            ...
            total += memmap[i].length;
        }
    }
    extern void pmm_set_boot_total_pages(uint32_t pages);
    pmm_set_boot_total_pages((uint32_t)(total / 4096));
}
#endif
