...
#else
/* No firmware-specific console; provide inert stubs to keep the build working. */
static void fallback_raw_putchar(struct flanterm_context *ctx, uint8_t c) {
    (void)ctx;
    (void)c;
}
...
static void noop_ctx(struct flanterm_context *ctx) {
    (void)ctx;
}

static void noop_ctx_size(struct flanterm_context *ctx, size_t value) {
    (void)ctx;
    (void)value;
}

static void noop_ctx_u32(struct flanterm_context *ctx, uint32_t value) {
    (void)ctx;
    (void)value;
}

static void noop_move_character(struct flanterm_context *ctx, size_t new_x, size_t new_y,
...
static void noop_scroll(struct flanterm_context *ctx) {
    (void)ctx;
}

static void noop_save_state(struct flanterm_context *ctx) {
    (void)ctx;
}

static void noop_restore_state(struct flanterm_context *ctx) {
    (void)ctx;
}

static void noop_double_buffer_flush(struct flanterm_context *ctx) {
    (void)ctx;
}

static void noop_full_refresh(struct flanterm_context *ctx) {
    (void)ctx;
}

static void fallback_deinit(struct flanterm_context *ctx, void (*free_fn)(void *, size_t)) {
    if (ctx == NULL || free_fn == NULL) {
        return;
    }

    free_fn(ctx, sizeof(struct flanterm_context));
}
...
void term_fallback(void) {
    ...
    struct flanterm_context *term = NULL;
    ...
    if (efi_boot_services_exited) {
        ...
        terms[0] = flanterm_fb_init(
            ext_mem_alloc, pmm_free,
            (uint32_t *)(uintptr_t)fb_fbs[0].framebuffer_addr, fb_fbs[0].framebuffer_width,
    ...
    term = terms[0];
    memset(term, 0, sizeof(struct flanterm_context));

    fallback_clear(NULL, true);

    term->set_text_fg = noop_ctx_size;
    term->set_text_bg = noop_ctx_size;
    term->set_text_fg_bright = noop_ctx_size;
    term->set_text_bg_bright = noop_ctx_size;
    term->set_text_fg_rgb = noop_ctx_u32;
    term->set_text_bg_rgb = noop_ctx_u32;
    term->set_text_fg_default = noop_ctx;
    term->set_text_bg_default = noop_ctx;
    term->set_text_fg_default_bright = noop_ctx;
    term->set_text_bg_default_bright = noop_ctx;
    term->move_character = noop_move_character;
    term->revscroll = noop_scroll;
    term->swap_palette = noop_ctx;
    term->save_state = noop_save_state;
    term->restore_state = noop_restore_state;
    term->double_buffer_flush = noop_double_buffer_flush;
    term->full_refresh = noop_full_refresh;
    term->deinit = fallback_deinit;
    ...
    term->set_text_fg_default_bright = fallback_set_text_fg_default;
    term->set_text_bg_default_bright = fallback_set_text_bg_default;
